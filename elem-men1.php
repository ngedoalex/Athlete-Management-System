              ...
              <table id="" class="" border="1" style="border-collapse: collapse; " width="100%">
                    <thead>   
                      ...
                      <tr>
                        <th colspan="<?php echo ($num*3)+1?>">ELEMENTARY ATHLETICS RESULT (BOYS) <a onclick='window.open("print.php?print=elem-boys","","width=1200,height=600");' class="btn btn-sm pull-right"><i class="fa fa-print"></i></a></th>
                      </tr>
                      ...
                      <tr>
                        ...
                        <?php }?>
                            ...
                            <tbody>
                        <?php
                        $q1=mysqli_query($con,"SELECT * from tbl_games where category='Boys' and level='Elementary' and event_id='$active_event_id'");
                      ...
                      <tr><th>Total</th>
                        <?php $q2=mysqli_query($con,"SELECT * from tbl_delegates WHERE event_id='$active_event_id' order by del_name ASC"); //find all delegates
                          while ($r2=mysqli_fetch_assoc($q2)) {
                             $gold=mysqli_query($con,"SELECT * from tbl_winner w inner join tbl_athlete a inner join tbl_games g where a.del_id='$r2[del_id]' and w.a_id=a.a_id and standing_id='Gold' and a.a_category='Boys' and g.game_id=w.game_id and g.level='Elementary'");
                            $rg=mysqli_fetch_assoc($gold);
                            $rgold=mysqli_num_rows($gold);

                            $silver=mysqli_query($con,"SELECT * from tbl_winner w inner join tbl_athlete a inner join tbl_games g where a.del_id='$r2[del_id]' and w.a_id=a.a_id and standing_id='Silver' and a.a_category='Boys' and g.game_id=w.game_id and g.level='Elementary'");
                             $rs=mysqli_fetch_assoc($silver);
                              $rsilver=mysqli_num_rows($silver);

                            $bronze=mysqli_query($con,"SELECT * from tbl_winner w inner join tbl_athlete a inner join tbl_games g where a.del_id='$r2[del_id]' and w.a_id=a.a_id and standing_id='Bronze' and a.a_category='Boys' and g.game_id=w.game_id and g.level='Elementary'");
                            $rb=mysqli_fetch_assoc($bronze);
